...
def seats_inp():
    ...
    if train == 'rj':
        if optn == 1:
            ...
            qu = "update seats_trains SET quantity={} where train='{}' and category='{}'".format(
                rj_A, 'RAJDHANI EXPRESS', 'CLASS A')
            s.execute(qu)
            sqcon.commit()
        if optn == 2:
            rj_B = rj_B-seat_inp
            ...
            qu = "update seats_trains SET quantity={} where train='{}' and category='{}'".format(
                rj_B, 'RAJDHANI EXPRESS', 'BUSINESS')
            s.execute(qu)
            sqcon.commit()
        if optn == 3:
            rj_E = rj_E-seat_inp
    ...
    if train == 'smj':
        if optn == 1:
            ...
            qu = "update seats_trains SET quantity={} where train='{}' and category='{}'".format(
                smj_A, 'SAMJHAUTA EXPRESS', 'CLASS A')
            s.execute(qu)
            sqcon.commit()
        if optn == 2:
            ...
            qu = "update seats_trains SET quantity={} where train='{}' and category='{}'".format(
                smj_B, 'SAMJHAUTA EXPRESS', 'BUSINESS')
            s.execute(qu)
            sqcon.commit()
        if optn == 3:
            ...
            qu = "update seats_trains SET quantity={} where train='{}' and category='{}'".format(
                smj_E, 'SAMJHAUTA EXPRESS', 'ECONOMY')
            s.execute(qu)
            sqcon.commit()
    ...
    if train == 'sht':
        if optn == 1:
            ...
            qu = "update seats_trains SET quantity={} where train='{}' and category='{}'".format(
                sht_A, 'SHATABDI EXPRESS', 'CLASS A')
            s.execute(qu)
            sqcon.commit()
        if optn == 2:
            ...
            qu = "update seats_trains SET quantity={} where train='{}' and category='{}'".format(
                sht_B, 'SHATABDI EXPRESS', 'BUSINESS')
            s.execute(qu)
            sqcon.commit()
        ...
        if optn == 3:
            ...
            qu = "update seats_trains SET quantity={} where train='{}' and category='{}'".format(
                sht_E, 'SHATABDI EXPRESS', 'ECONOMY')
            s.execute(qu)
            sqcon.commit()
    sttic = ""
...
def write_in_db(un, ps, em, ph):
    ...
    data = g.fetchall()
    sno = g.rowcount
    q = "insert into login_digirail values({},'{}','{}','{}','{}')".format(
        sno, un, ps, em, ph)
...
# __main__
sqcon = sq.connect(host='localhost', user='YourUsername',
                   passwd='YourPassword', database='DigiRail')
